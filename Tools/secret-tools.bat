...
echo.
echo   =============================================================================================
echo                              SECRET-TOOLS : WINRE RECOVERY CONSOLE
...
# Shows the standard Windows UAC consent prompt exactly once per launch —
# whether started from the desktop shortcut, PATH command, or directly.
if (-not $isWinRE) {
    $currentIdentity = [Security.Principal.WindowsIdentity]::GetCurrent()
    $currentPrincipal = New-Object Security.Principal.WindowsPrincipal($currentIdentity)
    if (-not $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        $selfBat = if (Test-Path "$toolsDir\secret-tools.bat") { "$toolsDir\secret-tools.bat" } else { "$installDir\secret-tools.bat" }
...
# is already gated by the UAC elevation Setup-Tools.bat performed.
